...
async function getBranches(params: Record<string, any> = {}) {
  let query = supabase.from('branches').select('*, rooms(*, tenants(id,status), bookings(id,status)), payments(amount,status)');
  ...
  return (data || []).map((branch: any) => {
    ...
    const reservedBeds = rooms.reduce((sum: number, room: any) => sum + (room.bookings || []).filter((b: any) => b.status !== 'OCCUPIED').length, 0);
    return {
      ...camel(branch),
      totalRooms,
      occupiedBeds,
...
async function getRooms(params: Record<string, any> = {}) {
  let query = supabase.from('rooms').select('*, branch:branches(*), tenants(*), bookings(id,status)');
  ...
  return (data || []).map((room: any) => {
    ...
    const reserved = (room.bookings || []).filter((booking: any) => booking.status !== 'OCCUPIED').length;
    return { ...camel(room), occupied: activeTenants.length, reserved, vacant: Math.max(0, Number(room.capacity) - activeTenants.length - reserved), tenants: camel(activeTenants) };
  });
}
...
async function getRoom(id: string) {
  const { data, error } = await supabase
    .from('rooms')
    .select('*, branch:branches(*), tenants(*), bookings(id,status)')
    .eq('id', id)
    .single();
  if (error) throw apiError(error.message);
  const activeTenants = (data.tenants || []).filter((tenant: any) => tenant.status === 'ACTIVE');
  const reserved = (data.bookings || []).filter((booking: any) => booking.status !== 'OCCUPIED').length;
  ...
  return {
    ...
    reserved,
    vacant: Math.max(0, Number(data.capacity) - activeTenants.length - reserved),
...
async function getBranchDashboard(id: string) {
  const { data: branch, error } = await supabase
    .from('branches')
    .select('*, rooms(*, tenants(id,status), bookings(id,status)), payments(*)')
    .eq('id', id)
    .single();
  if (error) throw apiError(error.message);
  ...
  const reservedBeds = rooms.reduce((sum: number, room: any) => sum + (room.bookings || []).filter((booking: any) => booking.status !== 'OCCUPIED').length, 0);
...
async function getDashboard() {
  const [branches, rooms, tenants, payments, admissions] = await Promise.all([
    ...
    supabase.from('rooms').select('*, tenants(id,status), bookings(id,status)'),
  ...
  return {
    ...
    vacantBeds: (rooms.data || []).reduce(
      (sum: number, room: any) => sum + Math.max(0, Number(room.capacity || 0) - (room.tenants || []).length - (room.bookings || []).filter((booking: any) => booking.status !== 'OCCUPIED').length),
      0
    ),
...
async function getAdmission(id: string) {
  const { data, error } = await supabase
    ...
    .select('*, branch:branches(*), payments(*), documents(*), booking:bookings(*)')
    .eq('id', id)
    .single();
  if (error) throw apiError(error.message);
...
async function getBookings() {
  const { data, error } = await supabase
    .from('bookings')
    .select('*, branch:branches(*), room:rooms(*), admissionApplication:admission_applications(*), tenant:tenants(*)')
...
async function createBooking(payload: any) {
  const { data, error } = await supabase.rpc('reserve_available_bed', {
    p_name: payload.name,
    p_phone: payload.phone,
    p_branch_id: payload.branchId,
    p_room_id: payload.roomId,
    p_expected_joining_date: payload.expectedJoiningDate,
    p_notes: payload.notes || null,
  });
  if (error) throw apiError(error.message);
  const booking = Array.isArray(data) ? data[0] : data;
  const { data: complete, error: completeError } = await supabase.from('bookings').select('*, branch:branches(*), room:rooms(*)').eq('id', booking.id).single();
  if (completeError) throw apiError(completeError.message);
...
async function routeGet(path: string, config: RequestConfig = {}) {
  ...
  if (path === '/notifications') {
    ...
    if (error) throw apiError(error.message);
    const notifications = camel(data || []);
    return { notifications, unreadCount: notifications.filter((item: any) => !item.isRead).length, alertsEnabled: true };
  }
  if (path === '/bookings') return getBookings();
...
async function routeDelete(path: string) {
  ...
  if (path.match(/^\/bookings\/[^/]+$/)) return supabase.from('bookings').delete().eq('id', path.split('/')[2]);
  if (path.match(/^\/notifications\/[^/]+$/)) return supabase.from('notifications').delete().eq('id', path.split('/')[2]);
