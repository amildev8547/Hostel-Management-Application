...
export const tenantEditSchema = z.object({
  body: z.object({
    ...
    whatsappNumber: z.string().regex(/^\d{10}$/).optional(),
    address: z.string().max(1000).optional(),
    guardianName: z.string().max(100).optional(),
    guardianPhone: z.union([z.literal(''), z.string().regex(/^\d{10}$/)]).optional(),
    nearestPoliceStation: z.string().optional(),
    occupation: z.string().optional(),
    workLocation: z.string().optional(),
    notes: z.string().max(2000).optional(),
    joiningDate: z.string().refine(isValidDateString).optional(),
    leavingDate: z.union([z.literal(''), z.string().refine(isValidDateString)]).optional(),
    profilePhoto: admissionImageSchema.optional(),
    aadhaarFront: admissionImageSchema.optional(),
    aadhaarBack: admissionImageSchema.optional(),
...
export const existingTenantSchema = z.object({
  body: z.object({
    ...
    guardianPhone: z.union([z.literal(''), z.string().regex(/^\d{10}$/, 'Guardian phone must be exactly 10 digits')]).optional(),
    ...
    notes: z.string().trim().max(2000).optional(),
    leavingDate: z.string().optional().refine((value) => !value || isValidDateString(value)),
    profilePhoto: admissionImageSchema.optional(),
    aadhaarFront: admissionImageSchema.optional(),
    aadhaarBack: admissionImageSchema.optional(),
